    ...
    async function salvar() {
        const schema = Yup.object().shape({
            ...
            tempo_ligado_2cv: Yup.string()
                .min(1)
                .required('Tempo ligado 2cv é obrigatório'),
            tempo_desligado_2cv: Yup.string()
                .min(1)
                .required('Tempo desligado 2cv é obrigatório'),
            tempo_ligado_5cv: Yup.string()
                .min(1)
                .required('Tempo ligado 5cv é obrigatório'),
            tempo_desligado_5cv: Yup.string()
                .min(1)
                .required('Tempo desligado 5cv é obrigatório'),
            acaoCorretiva: Yup.string()
                .min(1)
                .required('Ação corretiva é obrigatório'),
        });
