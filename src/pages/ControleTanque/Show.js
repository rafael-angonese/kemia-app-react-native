...
const Show = ({ route }) => {
    ...
    async function excluir() {
        ...
        setLoading(true);
        try {
            const response = await api.delete('/controle-tanques/' + item.id);
            setLoading(false);
            refresh();
    ...
    return (
        <View style={styles.container}>
            ...
            <Text>Tempo ligado 2,5cv: {item.tempo_ligado_2cv}</Text>
            <Text>Tempo desligado 2,5cv: {item.tempo_desligado_2cv}</Text>
            <Text>Tempo ligado 5cv: {item.tempo_ligado_5cv}</Text>
            <Text>Tempo desligado 5cv: {item.tempo_desligado_5cv}</Text>
            <Text>Ação Corretiva: {item.acao_corretiva}</Text>
            <Text>Tanque: {item.tanque.nome}</Text>
            <View style={styles.container_row}>
                <Button
                    ...
                    mode="contained"
                    onPress={() =>
                        navigation.navigate('ControleTanqueEdit', {
                            item: item,
