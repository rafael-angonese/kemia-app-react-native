...
const Show = ({ route }) => {
    ...
    async function excluir() {
        ...
        setLoading(true);
        try {
            const response = await api.delete('/controle-vazaos/' + item.id);
            setLoading(false);
            refresh();
    ...
    return (
        <View style={styles.container}>
            ...
            <Text style={styles.title_view}>Vazão</Text>

            <Text>Data: {formatDate(item.data)}</Text>
            <Text>Hora: {item.hora.slice(0, -3)}</Text>
            <Text>Vazão/Dia (V x TAB): {item.vazao_dia}</Text>
            <View style={styles.container_row}>
                <Button
                    ...
                    mode="contained"
                    onPress={() =>
                        navigation.navigate('ControleVazaoEdit', {
                            item: item,
