...
import { TextInput, Snackbar, Checkbox, Button, HelperText } from 'react-native-paper';
...
const New = ({ route }) => {
    ...
    async function salvar() {
        ...
        setLoading(true);
        try {
            const response = await api.post('/etas', {
                nome,
                descricao,
                is_vazao,
                is_ph,
            ...
            setIs_ph(false);
            setIs_pac(false);
            setIs_polimero(false);
            setIs_hipoclorito(false);
